...
def article_created(instance, created, **kwargs):
    ...
    for recipient in recipients:
        if created:
            notify.send(sender=instance.author, verb='article_created',
                        description="{} published a new article in '{}'".format(
                            instance.author.username, instance.tagList), recipient=recipient)
...
def commented_on(instance, created, **kwargs):
    ...
    """
    if created:
        notify.send(sender=instance.author, recipient=instance.article.author, verb='comment_created',
